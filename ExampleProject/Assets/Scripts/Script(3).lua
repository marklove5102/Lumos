...
function CreatePlayer()
    ...
    player:AddSprite(Vec2.new(-0.4, -0.4), Vec2.new(0.8, 0.8), Vec4.new(1.0,1.0,1.0,1.0)):SetTexture(texture)

    local params = RigidBodyParameters.new()
    params.position = Vec3.new( 2.75, 2.75, 1.0)
    params.scale = Vec3.new(0.4, 0.4,0.4)
	params.shape = Shape.Cube
	params.isStatic = false

    player:GetTransform():SetLocalPosition(Vec3.new(3.0,3.0,4.0))
...
function OnInit()
	...
	SetB2DGravity(Vec2.new(0.0,0.0))		---18.0))
    CreatePlayer()
...
--    gameOverEntity:AddSprite(Vec2.new(0.0, 0.0), Vec2.new(30, 4), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(gameOverTexture)
--	gameOverPos = player:GetTransform():GetWorldPosition() + Vec3.new(15.0, 2.0, 1.0)
...
--    *   "+" for closed door
		if row == "#" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(9,2), Vec2.new(16, 16), Vec2.new(16, 16), 1)

			local params = RigidBodyParameters.new()
		    params.position = Vec3.new( k, j, -3)
		    params.scale = Vec3.new(0.5, 0.5,0.5)
			params.shape = Shape.Cube
			params.isStatic = false
		...
		elseif row == "." then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(16,14), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		elseif row == "<" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(16,2), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		elseif row == ">" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(20,2), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		elseif row == "%" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(13,11), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		elseif row == "*" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(10,12), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		elseif row == "'" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(22,4), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		elseif row == "+" then
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(22,6), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		else
			tile:AddSprite(Vec2.new(-0.5, -0.5), Vec2.new(1, 1), Vec4.new(1.0, 1.0, 1.0, 1.0)):SetTexture(SpriteSheetTexture)
			tile:GetSprite():SetSpriteSheet(Vec2.new(9,7), Vec2.new(16, 16), Vec2.new(16, 16), 1)
		end
		tile:GetTransform():SetLocalPosition(Vec3.new(k, j, -3))
		tile:GetTransform():SetLocalScale(Vec3.new(1, 1, 1))
      end
  end
...
function OnUpdate(dt)
	...
	phys:GetRigidBody():SetLinearVelocity(Vec2.new(vel.x + speed * dx * dt, vel.y + speed * dy * dt))
	phys:GetRigidBody():SetOrientation(0.0)

	local cameraPos = Vec3.new(pos.x, pos.y, -4.0)
